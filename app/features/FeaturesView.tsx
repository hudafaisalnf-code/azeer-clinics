...
export default function FeaturesView({ sanityData }: { sanityData: SanityData }) {
  const { t } = useLang();
  const f = t.features;
  ...
  const list = sd?.list?.length ? sd.list : f.list;
  const highlightTitle = sd?.highlightTitle || f.highlightTitle;
  const highlightDesc = sd?.highlightDesc || f.highlightDesc;
  const highlightPoints = sd?.highlightPoints?.length ? sd.highlightPoints : f.highlightPoints;
  ...
  return (
    <>
      ...
      <CtaBand title={ctaTitle} button={ctaButton} />
    </>
  );
}
